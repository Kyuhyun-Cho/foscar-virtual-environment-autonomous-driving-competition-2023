...
class mpc_path_tracking:
    ...
    def controller(self, car_state, ref_path, iter_num=5, debug=False):
        ...
        # print("1 parameter called car_state in controller",type(car_state))
        # print(car_state)
    ...
    def iterative_solver(self, car_state, ref_path, cur_vel_array, iter_num = 5):
        ...
        for i in range(iter_num):
            # print("2 parameter called car_state in interative_solver",type(car_state))
            # print(car_state)
    ...
    def convx_optimizer(self, car_state, ref_traj, cur_vel_array, s_array_bar):
        ...
        car_state_cvx = cvxpy.Parameter(shape=(3,1))
        car_state_cvx.value = car_state.reshape((3,1))
        # print(indep_s[:, 0])
        # print(car_state_cvx)
        constraints += [ indep_s[:, 0].reshape((3,1)) == car_state_cvx ]   # init state
    ...
    def state_predict(self, car_state, cur_vel_array):
        ...
        car_state = np.array(car_state, ndmin=2)

        if car_state.shape[0] == 1:
            car_state = car_state.T

        # print(s_array.shape, car_state[:, 0])
        ...
        for i in range(self.receding):
            # print("3 parameter called car_state in state_predict",type(car_state))
            # print(car_state)
    ...
    def motion_predict_model(self, car_state, vel, wheel_base, sample_time):
        ...
        # print("4 parameter called car_state in motion_predict_model",type(car_state), car_state.shape)
        phi = car_state[2, 0]
